...
QDispersion::~QDispersion() { delete ui; }
...
void QDispersion::openFile(const QString &filename) {
  ...
  try {
    ...
    ui->filename->setText(
        QString::fromStdString(filename.toStdString().substr(pos + 1)));
    ...
    if (ndiv.size() != 0 && ndiv.size() == labels.size() - 1) {
      ...
      for (auto c = children1.begin(); c != children1.end(); ++c) {
        ...
        if (id > 0) {
          reinterpret_cast<QLineEdit *>(*c)->setText(
              QString::fromStdString(labels[id - 1]));
        }
      }
    ...
    bool electronic =
        (dynamic_cast<EigParserElectrons *>(_eigparser.get()) != nullptr);
...
void QDispersion::plot(bool commandOnly) {
  ...
  if (ui->saveCommand->isChecked()) {
    command = ":save ";
    save = Graph::PRINT;
  } else if (ui->dataCommand->isChecked()) {
    command = ":data ";
    save = Graph::DATA;
  }
  command += "band ";
  command +=
      QString::fromStdString(_eigparser->getFilename()).replace(" ", "\\ ") +
      " ";

  // Unit
  UnitConverter eunit = UnitConverter::getFromString(
      ui->energyUnit->currentData().toString().toStdString());
  ...
  if (commandTmp[commandTmp.size() - 1] == ':')
    commandTmp.chop(1);
  if (hasNlabels == hasNdiv + 1 && hasNdiv > 0)
    command += commandTmp;

  // fermi
  if (std::abs(ui->fermi->value()) > 1e-7) {
    command += " fermi ";
    UnitConverter fermiUnit = UnitConverter::getFromString(
        ui->fermiUnit->currentData().toString().toStdString());
    fermiUnit = UnitConverter::Ha;
    command += QString::number(ui->fermi->value() * fermiUnit, 'g', 14);
  }

  // ignore
  if (ui->ignore->isEnabled() && ui->ignore->value() > 0)
  ...
  if (ui->fatbands->isChecked()) {
    ...
    if (ui->angular->isEnabled() && ui->angular->currentIndex() != 5) {
      ...
      if (ui->p4->isChecked())
        commandMagnetic += " +4";
      if (!commandMagnetic.isEmpty())
        command += " magnetic " + commandMagnetic;
    }
  }
  ...
  if (!commandOnly) {
    ...
    try {
      ...
      Graph::Config conf;
      conf.save = save;
      Graph::plotBand(*_eigparser.get(), parser, ui->plot, conf);
      if (save == Graph::PRINT) {
        Gnuplot gplt;
        Graph::plotBand(*_eigparser.get(), parser, &gplt, conf);
      }
...
void QDispersion::coordStatusBar(QMouseEvent *event) {
  ...
  QString info = "kpt=" + QString::number(x) + " E=" + QString::number(y) +
                 QString::fromStdString(ui->energyUnit->currentUnit().str());
...
void QDispersion::setSpacerSize() {
  ...
  ui->ndivLeft->changeSize(actualSize.width(), actualSize.height(),
                           QSizePolicy::Fixed);
  ui->ndivRight->changeSize(actualSize.width(), actualSize.height(),
                            QSizePolicy::MinimumExpanding);
}

void QDispersion::on_fermi_editingFinished() { QDispersion::plot(); }
...
void QDispersion::on_buttonBox_clicked(QAbstractButton *button) {
  ...
  if (b == QDialogButtonBox::Save) {
    ...
    QString filename =
        QFileDialog::getOpenFileName(this, tr("Dispersion file"), "", "",
...
void QDispersion::on_plotCommand_clicked(bool checked) { this->plot(checked); }

void QDispersion::on_saveCommand_clicked(bool checked) { this->plot(checked); }

void QDispersion::on_dataCommand_clicked(bool checked) { this->plot(checked); }
